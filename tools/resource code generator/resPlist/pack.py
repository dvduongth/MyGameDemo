...
g_root_path = "../../../"
g_resource_output_folder = os.path.join(g_current_folder, g_root_path + 'src/resources') #for copy action
g_data_code_folder = os.path.join(g_current_folder, g_root_path + 'src/resources')
g_data_code_file = "ResourcePlist.js"
    ...
    for folder_child in os.listdir(folder):
        ...
        if os.path.isfile(file_path):
            ...
            if(any(file_ext in ext for ext in g_list_ext)):
                ...
                relative_path_for_id = relative_path_for_id.replace('\\', '/').replace('/', '__')
                ...
                if(data.id in g_list_check_data):
                    g_list_data.append(data)
                else:
                    g_list_check_data.append(data.id)

            else:
                if(any(file_ext in ext for ext in g_list_ext_0)):
                    relative_path_for_id = os.path.relpath(file_path, g_resource_input_folder)
                    relative_path_for_id = os.path.splitext(relative_path_for_id)[0] #todo get file_name
                    relative_path_for_id = relative_path_for_id.replace('\\', '/').replace('/', '__')
                    # print "find_resources_from_folder child relative_path_for_id: " + relative_path_for_id
                    ...
                    data = ResourceFormatData()
                    # if len(g_list_ext_0) > 1:
                    #     data.id = relative_path_for_id + "_" + file_ext[1:]
                    # else:
                    data.id = relative_path_for_id
                    data.path = relative_path_for_path.replace(file_ext, g_list_ext[0])
                    if(data.id in g_list_check_data):
                        g_list_data.append(data)
                    else:
                        g_list_check_data.append(data.id)
